...
#make summmer graphs just like flbs one ####
...
#linear regression for trends --- this is not the analysis used in final version
...
#graphs----THIS GRAPH DID NOT GET USED IN FINAL VERSION
ggplot()+
  #geom_hline(yintercept = 84.45, linetype = "dashed", color = "black", alpha = 0.5)+
...
#make graphs for this-- NOT USED IN FINAL VERSION
ggplot()+
  #geom_hline(yintercept = 84.45, linetype = "dashed", color = "black", alpha = 0.5)+
...
#make graphs for this---NOT USED IN FNAL VERSION
ggplot()+
  #geom_hline(yintercept = 84.45, linetype = "dashed", color = "black", alpha = 0.5)+
...
ggplot(data = wfsummerbothnd %>% filter(Characteristic_ID == "TP"), 
  ...
  geom_hline(yintercept = 3.59, linetype = "dashed", color = "black", alpha = 0.7)+
  geom_point(size = 2.5)+
  geom_errorbar(aes(ymin = mean-se, ymax = mean+se), width = 0.3)+
  #ylim(0, 40)+
  mlc_theme+
...
#below is only old code that was not used in final version -----------------------------------------------
...
#OLD CODE !!! code that did not end up being used to make final version #####
#IF THIS DOESNT GET USED MOVE IT DOWN ####
#make mean for each year and max and min
...
#graphs for both sites combined that did not get used
...
#ND  graph for summer chl-a combining both sites ####
